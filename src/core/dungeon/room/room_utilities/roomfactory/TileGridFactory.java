...
class TileGridFactory extends Factory<TileGrid>{
    private final String name = "dungeonTileSet";
    private final int layers = 5;
    private final int tileN = 18;
    private final int tileM = 16;
    private final Tile tileMap[][] = new Tile[tileN][tileM];
    private final HashSet<Integer> collisionTiles = new HashSet<>(
    ...
    private final int direct2[][] = { { -1, -1 }, { 1, -1 }, { -1, 1 }, { 1, 1 } };

    private Tile getTile(int r, int c) {
        // the r and c are backwards because I messed something up and it would take 6 hours to fix so i'm keeping it like this
        return new Tile(ImageUtilities.getImage("tiles", name, c, r), collisionTiles.contains(tileN * c + r), false);
    }

    private final HashSet<Integer> layer0 = new HashSet<>(Arrays.asList(5, 6));
    ...
    private final HashSet<Integer> layer1 = new HashSet<>(Arrays.asList(1, 2, 3, 4, 6));
    ...
    private final HashSet<Integer> layer2 = new HashSet<>(Arrays.asList(1, 2, 4, 5, 6));
    final int[] smallRocks = { 6, 24, 42 };
    final int[] mediumRocks = {};
